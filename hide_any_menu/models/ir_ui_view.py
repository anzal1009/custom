        ...
        for hide_button_config in self.env['ir.model']._get(name_manager.model._name).mapped('hide_button_config_ids'):
            if hide_button_config.button_hide_by in ['method', 'action_id', 'field']:
                for elem in node.iter(tag='field'):
                    if elem.get('name') == hide_button_config.button_hide_by_statement:
                        hide = [hide_button_config]
                        break
                if node.get('name') == hide_button_config.button_hide_by_statement:
                    hide = [hide_button_config]
                    break
                ...
                for elem in node.iter(tag='span'):
                    if elem.text and hide_button_config.button_hide_by_statement in elem.text:
                        hide = [hide_button_config]
                        break
                if node.get('string') == hide_button_config.button_hide_by_statement:
                    hide = [hide_button_config]
                    break
                ...
                break
        if hide:
            if not self.env.user.groups_id & hide[0].group_ids:
                return None
            node.set('invisible', '1')
            node_info['modifiers']['invisible'] = True
        return None
