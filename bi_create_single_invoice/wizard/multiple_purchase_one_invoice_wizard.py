...
class multiple_purchase_one_invoice_wizard(models.TransientModel):
    ...
    _description="multiple purchase one invoice wizard"
    ...
    def create_invoices_for_purchase_order(self):
        ...
        payment_term_id = [order.payment_term_id for order in purchases]
        ...
        valid_purchase_orders = purchase_order_obj.search([('partner_id','=',partner_ids[0].id),('state','in',['done','purchase'])]).ids
        for po in purchases:
            if po.id not in valid_purchase_orders :
                raise UserError(_("All Purchase Order should be confirmed"))
        origin_list  = [order.name for order in purchases if order.name]
        origin = ','.join(origin_list)

        purchase_order_lines = purchase_order_line_obj.search([('order_id','in',purchase_ids)])
        account_id = partner_ids[0].property_account_payable_id.id or False
        ...
        for purchase_line in purchase_order_lines:
            ...
            if purchase_line.product_id.id:
                invoice_line_account_id = purchase_line.product_id.property_account_expense_id.id or purchase_line.product_id.categ_id.property_account_expense_categ_id.id or False
            if not invoice_line_account_id:
                inc_acc = ir_property_obj.get('property_account_expense_categ_id', 'product.category')
                invoice_line_account_id = purchase_line.order_id.fiscal_position_id.map_account(inc_acc).id if inc_acc else False
            if not invoice_line_account_id:
                raise UserError(
            ...
            taxes = purchase_line.taxes_id.filtered(lambda r: not purchase_line.company_id or r.company_id == purchase_line.company_id)
            tax_ids = []
