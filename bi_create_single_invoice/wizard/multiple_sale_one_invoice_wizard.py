...
class multiple_sale_one_invoice_wizard(models.TransientModel):
    ...
    _description="multiple sale one invoice wizard"
    ...
    def create_invoices_for_sale_order(self):
        ...
        payment_term_id = [order.payment_term_id for order in sales]
        ...
        valid_sale_orders = sale_order_obj.search(
            [('partner_id', '=', partner_ids[0].id), ('state', 'in', ['sale', 'done'])]).ids

        for sale in sales:
            if sale.id not in valid_sale_orders:
                raise UserError(_("All Sale Order should be confirmed"))
