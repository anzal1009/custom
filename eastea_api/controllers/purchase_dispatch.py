...
class PurchaseDispatch(http.Controller):

    @http.route('/inv/Purchase', type='json', csrf=False, auth='public')
    def invoicePurchase(self, **rec):
        print(rec)
        po_numbers = []
        for row in rec["data"]:
                ...
                if product_item:
                    ...
                    if product:
                        ...
                        lot_no = request.env['stock.production.lot'].sudo().search(
                            [('company_id', '=', to_company_detail2.id), ('name', '=', product_lot_number)])
                        if not lot_no:
                            lot_number = {
                ...
                if purchase_order_1:
                    ...
                    if purchase_order_1.picking_ids:
                        for picking in purchase_order_1.picking_ids:
                            lot_no = request.env['stock.production.lot'].sudo().search(
                                [('company_id', '=', to_company_detail2.id), ('name', '=', product_lot_number)])
                            picking.move_line_ids.lot_id = lot_no.id
                            picking.move_line_ids.lot_name = lot_no.name
